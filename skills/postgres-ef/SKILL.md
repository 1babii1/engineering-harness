---
name: postgres-ef
description: Use for PostgreSQL, Npgsql, Dapper, and EF Core rules - SQL design, index direction, migrations, transactions, query reviews. Use when adding a migration, an index, or reviewing a query against this repository's schema-per-service Postgres.
---

# PostgreSQL / Dapper / EF Core

- Design from access patterns, not abstract normalization alone.
- Parameterize every query.
- Select only required columns.
- Use indexes for demonstrated filters, joins, sorting, and access patterns.
...
- **Prove the upgrade path:** apply the migration to a database holding representative historical
  data (a restored snapshot or a seeded fixture), not only to an empty one.
