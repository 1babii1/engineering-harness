...
`tdd` owns the red-green-refactor loop and where seams go. This skill owns proving that a test
that passed actually catches the thing it claims to guard - the step most often skipped once the
loop is green.
...
- a concurrency/idempotency guard asserted against a brand-new aggregate - seed it against one
  that already has related rows, since a fresh primary key can mask a missing constraint on the
  row that actually needed protecting;
