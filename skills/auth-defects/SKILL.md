---
name: auth-defects
description: Checks that catch real authentication/authorization defects - enumeration, session revocation on credential change, external-identity linking, step-up, key rotation, passwords, passkeys, email change, account deletion. Use when designing, changing, or reviewing login, registration, password reset, MFA/passkeys, sessions, OAuth2/OIDC tokens, external identity providers, or account deletion.
---
...
Require negative tests for each applicable check above - the attacker, the wrong client, the
expired/tampered token, the unverified email - alongside the happy path, and a real HTTP flow
where runnable. Call the Skill tool with "test-evidence" before counting any of these tests as
proof: a regression test never watched to fail against the broken state is not evidence.
